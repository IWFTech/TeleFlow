...
public sealed class TelegramSchemaTests
{
    ...
    {
        var metadata = ReadGeneratedSchemaMetadata();
        ...
        AssertGeneratedHeader(typeFile, "Type", metadata);
        AssertGeneratedHeader(methodFile, "Method", metadata);
        AssertGeneratedHeader(responseFile, "Response", metadata);
        AssertGeneratedHeader(abstractionFile, "Abstraction", metadata);
        AssertGeneratedHeader(constantsFile, "ConstantGroup", metadata);
    }
    ...
    public void GeneratedManifest_ContainsSnapshotAndPipelineMetadata()
    {
        ...
        Assert.Matches("^[0-9]+(?:\\.[0-9]+)+$", telegramBotApi.GetProperty("version").GetString()!);
        Assert.Matches("^[0-9]{4}-[0-9]{2}-[0-9]{2}$", telegramBotApi.GetProperty("releasedAt").GetString()!);
        Assert.Matches("^[a-z]+-[0-9]{1,2}-[0-9]{4}$", telegramBotApi.GetProperty("changelogAnchor").GetString()!);
        Assert.Equal(
            $"https://core.telegram.org/bots/api-changelog#{telegramBotApi.GetProperty("changelogAnchor").GetString()}",
            telegramBotApi.GetProperty("changelogUrl").GetString());
        Assert.True(pipeline.GetProperty("schemaVersion").GetInt32() > 0);
        Assert.True(pipeline.GetProperty("generatorVersion").GetInt32() > 0);
    }
    ...
    private static void AssertGeneratedHeader(
        string fileContents,
        string kind,
        GeneratedSchemaMetadata metadata)
    {
        ...
        Assert.Contains($"//   Telegram Bot API version: {metadata.Version}", fileContents);
        Assert.Contains($"//   Telegram Bot API release: {metadata.ReleaseDate}", fileContents);
        Assert.Contains($"//   Telegram Bot API changelog: {metadata.ChangelogUrl}", fileContents);
    ...
    private static GeneratedSchemaMetadata ReadGeneratedSchemaMetadata()
    {
        var manifestPath = Path.Combine(RepositoryRoot, "src", "TeleFlow.Telegram.Schema", "telegram-bot-api.manifest.json");

        using var document = JsonDocument.Parse(IoFile.ReadAllText(manifestPath));
        var telegramBotApi = document.RootElement.GetProperty("telegramBotApi");

        return new GeneratedSchemaMetadata(
            telegramBotApi.GetProperty("version").GetString()!,
            telegramBotApi.GetProperty("releasedAt").GetString()!,
            telegramBotApi.GetProperty("changelogUrl").GetString()!);
    }

    private sealed record GeneratedSchemaMetadata(
        string Version,
        string ReleaseDate,
        string ChangelogUrl);
