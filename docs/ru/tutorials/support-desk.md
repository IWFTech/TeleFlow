...
Этот tutorial показывает реалистичную форму TeleFlow bot. Пользователь создаёт support ticket, а admin берёт или закрывает его через inline buttons.
...
- typed callbacks;
- inline keyboards;
- прямые `ctx.Bot.*Async` calls;
- cancellation tokens.

Storage здесь in-memory, потому что цель - показать framework usage, а не production persistence.
...
В реальном приложении inline keyboard для admin notification можно собрать рядом с handler или вынести serializer в service. Здесь direct `ctx.Bot` style call показывает, что low-level API доступен.
...
- Handlers остаются маленькими.
- Repositories и services - обычные DI services.
- State используется только для user wizard.
- Admin actions используют typed callbacks.
- Telegram API доступен через `ctx.Bot`.
- Memory storage и in-memory repositories - demo choices, не production persistence.

Для production замени storage, добавь tests, используй configuration binding и документируй admin access rules.
