...
Exact command и command-template matching приводят настроенный pattern и
входной command body к Unicode NFC, а затем применяют ordinal,
culture-independent настройку `IgnoreCase`. Поэтому канонически эквивалентные
последовательности, например составная `ё` и `е` с combining diaeresis,
матчатся как одна command. Строковые values из `[CommandTemplate]` приходят в
NFC.

Исходный message text в `MessageContext` не меняется. TeleFlow не считает
разные буквы `е` и `ё` алиасами. Если нужны оба написания, объяви обе routes
явно.

Text routes и regex routes сохраняют input contract приложения и не применяют
неявную Unicode normalization. Для regex route с canonical-equivalence
семантикой нормализацию нужно задать явно.
